"""
Spatial Geometry — deterministic spatial relation verification from bounding boxes.

Carried over from relcheck_v2/spatial.py. Zero hallucination risk — pure
geometry, no model involved.
"""
...
# ── Constants (inlined from v2 config to keep this self-contained) ───────
...
def spatial_verdict(
    ...
    if r in ("near", "next to", "beside"):
        ...
        return rel_dist < NEAR_DISTANCE_THRESHOLD
...
def compute_spatial_facts(
    ...
    Args:
        detections: List of (label, score, bbox) tuples.
...
# ── Core noun extraction (spaCy, matching v2 entity.core_noun) ───────────
...
def core_noun(phrase: str) -> str:
    ...
    Identical to relcheck_v2.entity.core_noun — lemmatized, lowercased.
